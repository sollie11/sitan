	...
	<div class="col100">
		<div class="card100">
			<div class="card-header">
				<div class="card-header-title">{{ __('Clients') }}</div>
<?php 
$aData = $oData['aData'];
...
@if ($oRec)
	...
	$iRowColor++;
	?>>
	<td>{{ $oRec->id }}</td><td>{{ $oRec->questionnaire }}</td><td>{{ $oRec->programme }}</td><td>{{ $oRec->business_name }}</td><td>{{ $oRec->email }}</td><td>{{ $oRec->updated_at }}</td>
	</tr>
@endif
@endfor
    </table>
