	...
	<div class="col100">
		<div class="card100">
			<div class="card-header">
				<div class="card-header-title">{{ __('New clients') }}</div>
<?php 
$aData = $oData['aData'];
...
@if ($oRec)
	...
	$iRowColor++;
	?>>
	<td>{{ $oRec->id }}</td><td>{{ $oRec->questionnaire }}</td><td>{{ $oRec->programme }}</td><td>{{ $oRec->email }}</td><td>{{ $oRec->first_name }}</td><td>{{ $oRec->surname }}</td><td>{{ $oRec->created_at }}
	</tr>
@endif
@endfor
    </table>
