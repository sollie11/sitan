	...
	<div class="col100">
		<div class="card100">
			<div class="card-header">
				<div class="card-header-title">{{ __('Questions Categories') }}</div>
<?php 
$aData = $oData['aData'];
...
$oSort = $oData['oSort'];
?>
				@include('grid', array('iPages' => $oPaging['iPages'], 'aButton' => array('/questions/categories/add', 'Add')))
				@include('alerts')
			</div>
...
@if ($oRec)
	...
	$iRowColor++;
	?>>
	<td>{{ $oRec->id }}</td><td>{{ $oRec->index_no }}</td><td>{{ $oRec->description }}</td>
	</tr>
@endif
@endfor
    </table>
